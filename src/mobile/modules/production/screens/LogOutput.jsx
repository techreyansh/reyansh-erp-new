// Log Output — pick an open WO → pick a routed stage → enter good output (+ reject)
// → ppc_post_jobcard through the offline outbox. cap: production.log.
...
export default function LogOutput({ api }) {
  ...
  const submit = async () => {
    if (!stage || Number(output) <= 0) return;
    setBusy(true);
    try {
      const { rpc, args } = logOutputIntent(stage, { output, reject });
      const idempotencyKey = newKey();
      await api.submit({ rpc, args, entity: 'open_wos', idempotencyKey });
      setFeed((f) => [{ id: idempotencyKey, title: `${stage.stage_name}: +${output}${Number(reject) > 0 ? ` (−${reject} rej)` : ''}`, at: Date.now() }, ...f].slice(0, 20));
      setSnack({ open: true, message: 'Output logged.', severity: 'success' });
      setOutput(''); setReject('');
    } catch (e) {
      setSnack({ open: true, message: e?.message || 'Could not log output.', severity: 'error' });
    ...
    <Box>
      ...
      {wo && (
        <Button variant="outlined" fullWidth onClick={() => setStagePicker(true)} disabled={!stages.length} sx={{ mb: 1.5, justifyContent: 'flex-start', py: 1.5 }}>
          {stage ? `${stage.stage_name} · ${stage.status}` : (stages.length ? 'Pick a stage' : 'No routed stages on this WO')}
        </Button>
      )}
      {stage && (
        <>
          <NumPad label={`Good output — ${stage.stage_name}`} value={output} onChange={setOutput} />
          <TextField label="Reject qty (optional)" type="number" value={reject} onChange={(e) => setReject(e.target.value)} fullWidth size="small" sx={{ mt: 1.5 }} />
        </>
      )}
      <Box sx={{ mt: 2 }}><RecentFeed items={feed} title="This session" emptyText="Nothing logged yet." /></Box>
      <SubmitBar label="Log Output" onSubmit={submit} busy={busy} disabled={!stage || Number(output) <= 0} />
