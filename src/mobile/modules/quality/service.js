// Quality module read helpers — open work orders + their stages + recent QC.
// Mirrors the Production module's reads and shares the same cache keys (open_wos,
// wo_stages:*) so pickers open offline from the last snapshot.
...
/** Recent QC checks for one WO (newest first). Cached per WO. */
export async function listQc(api, woId) {
  if (!woId) return [];
  const cacheKey = `wo_qc:${woId}`;
  if (isOnline()) {
    try {
      const { data, error } = await supabase
        .from('ppc_wo_qc')
        .select('id, check_type, result, value, created_at')
        .eq('wo_id', woId)
