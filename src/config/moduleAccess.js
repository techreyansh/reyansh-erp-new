...
const ROUTE_MODULE_RULES = [
  ...
  { test: (path) => path === '/plant-command' || path === '/production-log' || path === '/production-intelligence' || path === '/assembly-operations' || path === '/job-cards' || path === '/mes-setup' || path === '/mes-dashboard' || path === '/production-demand' || path.startsWith('/ppc') || path.startsWith('/cable-production') || path === '/cable-floor' || path.startsWith('/molding') || path === '/molding-production', moduleKey: MODULE_KEYS.PRODUCTION },
