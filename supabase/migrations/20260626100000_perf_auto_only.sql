-- Performance "for now": remove MANAGER and MEETING scoring. The score is now
-- 100% auto-calculated from actual ERP work — tasks done, on-time, checklist,
-- workflow, production yield. The perf_reviews row is read only for the week lock
-- + manager remarks (a note), never for scoring. Managers no longer set scores.
...
DECLARE
  ...
  c_work numeric; c_ontime numeric; c_chk numeric; c_wf numeric; c_prod numeric;
  w_work numeric := 0.40; w_ontime numeric := 0.25; w_chk numeric := 0.15; w_wf numeric := 0.10; w_prod numeric := 0.15;
  num numeric := 0; den numeric := 0; v_score numeric; v_band text;
  v_rev public.perf_reviews;
BEGIN
  -- TASKS due in the week (by effective due date)
  ...
  -- PRODUCTION: cable stages completed this week, yield% = good/(good+scrap)
  ...
  -- perf_reviews kept ONLY for the week lock + manager remarks (no manual scoring).
  SELECT * INTO v_rev FROM public.perf_reviews WHERE lower(employee_email)=v_email AND week_start=p_week_start;

  -- Renormalize over the auto-calculated categories that have data. Manual
  -- MEETING + MANAGER scores are intentionally excluded.
