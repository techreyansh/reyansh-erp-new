-- NPD Intelligence v2 — extends npd_intel_summary (same signature) with two
-- additional bundle keys: dev_type_breakdown (active projects by development_type)
-- and rework (resample/reject loops + reworked-project counts). Additive: all
-- existing keys are unchanged. Read-only, no data/schema change.
BEGIN;
...
as $function$
  ...
  select jsonb_build_object(
    ...
    -- v2: active developments by type
    'dev_type_breakdown', coalesce((select jsonb_agg(jsonb_build_object('dev_type', dt, 'count', c) order by c desc)
      from (select coalesce(development_type, '(unspecified)') dt, count(*) c
            from proj where status = 'active' group by development_type) t), '[]'::jsonb),
    -- v2: rework / resample loop signal
    'rework', jsonb_build_object(
      'resample_loops', (select count(*) from public.npd_feedback f join proj p on p.id = f.project_id
                         where f.outcome = 'resample'),
      'rejected', (select count(*) from public.npd_feedback f join proj p on p.id = f.project_id
                   where f.outcome = 'rejected'),
      'reworked_projects', (select count(*) from proj where coalesce(revision, 1) > 1),
      'avg_revision', (select round(avg(coalesce(revision, 1))::numeric, 2) from proj)
    ),
