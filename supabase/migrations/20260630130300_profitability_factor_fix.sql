-- Profitability V1.2 fix: material_factors must compare ACTUAL consumption against
-- the standard FOR THE PRODUCED QTY (rate/wastage variance), not against expected
-- over all sales — otherwise the qty mismatch hides overruns. Also link the demo
-- invoice to its sales order so by_order populates.
BEGIN;
...
begin
  ...
  -- expected material BY GROUP for the PRODUCED qty (matched to actual = rate/wastage variance)
  expf as (
    select public.prof_material_group(cl.material_code) grp,
           sum(cl.amount / nullif(cv.qty_basis,0) * wq.act_qty) amt
    from (
      select pr.id pid,
        (select id from public.costing_version where product_id=pr.id and status='released' order by version_number desc limit 1) cvid,
        sum(w.produced_qty) act_qty
      from public.ppc_wo w join public.product pr on pr.ppc_item_id=w.item_id
      where w.created_at::date between p_from and p_to and w.status in ('qc','done')
      group by pr.id
    ) wq
    join public.costing_version cv on cv.id = wq.cvid
...
-- demo: link the invoice to the WO's sales order so by_order shows actual revenue.
create or replace function public.profit_seed_actual_demo()
returns jsonb language plpgsql security definer set search_path to 'public' as $fn$
declare
  v_loc uuid; v_p record; v_item uuid; v_wo uuid; v_inv uuid; v_soline uuid; v_so uuid;
...
begin
  ...
  loop
    ...
    select l.id, l.so_id into v_soline, v_so from public.sales_order_line l where l.product_id=v_p.id limit 1;
    ...
    insert into public.finance_invoices (invoice_number, customer_code, customer_name, invoice_date, amount, status, sales_order_id)
    values ('PIC-DEMO-INV-'||n, 'PICDEMOC1', 'Acme Appliances', current_date-8, round(v_qty*v_sp,2), 'ISSUED', v_so) returning id into v_inv;
