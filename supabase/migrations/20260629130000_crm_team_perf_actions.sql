-- Team Performance: credit reps for ACTION work on accounts they don't own.
-- Adds an `actions_assigned` metric = count of accounts where the rep is the
-- next_action_owner but NOT the account owner. Additive CREATE OR REPLACE; the
-- existing fields are unchanged, so CrmTeam.js keeps working.
BEGIN;
...
as $function$
  ...
  act as (
    select lower(next_action_owner_email) as rep, count(*) as actions_assigned
    from public.crm_pipeline
    where next_action_owner_email is not null and btrim(next_action_owner_email) <> ''
      and lower(coalesce(next_action_owner_email,'')) <> lower(coalesce(owner_email,''))
    group by lower(next_action_owner_email)
  )
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'actions_assigned', coalesce(act.actions_assigned,0),
  ...
  from agg a
  left join fu  on fu.owner_email = a.owner_email
  left join act on act.rep = lower(a.owner_email);
$function$;
