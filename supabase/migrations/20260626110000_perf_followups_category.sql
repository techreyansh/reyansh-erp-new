-- Performance: add a FOLLOW-UPS category, scored from the employee's OWN CRM
-- follow-ups (crm_pipeline_activity owned by them, due this week, completed on
-- time vs next_follow_up_date). This is where the strict ownership work pays off
-- — the score counts only the follow-ups that belong to that employee.
...
BEGIN
  ...
  -- FOLLOW-UPS owned by this employee, due this week, completed on time.
  SELECT count(*),
         count(*) FILTER (WHERE status='completed' AND completed_at IS NOT NULL AND completed_at::date <= next_follow_up_date)
    INTO v_fu_due, v_fu_ok
  FROM public.crm_pipeline_activity
  WHERE lower(coalesce(owner_email,''))=v_email
    AND next_follow_up_date BETWEEN p_week_start AND v_end;
