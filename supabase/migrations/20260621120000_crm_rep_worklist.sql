-- CRM Rep Daily Worklist + RFM segmentation.
-- A "who do I contact today, and why" engine over the unified client master
-- (crm_pipeline account_type='client'), combining:
--   * reorder cadence (crm_customer_analytics: due / overdue / churn)
--   * AR overdue (v_ar_invoices)
--   * open follow-ups (crm_pipeline_activity.next_follow_up_date + pipeline.next_action_date)
--   * recency of contact (last_contact_date)
-- Produces a priority score (0-100), an RFM segment, and a reasons[] list per
-- account. Read-only RPC; SECURITY DEFINER with an owner scope param so reps see
-- their own + unassigned accounts, CEO/managers (p_owner_email NULL) see all.
BEGIN;
...
RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  ...
  joined AS (
    SELECT acct.id, acct.customer_code, acct.company_name, acct.owner_email,
           ...
           fu.followup_subject, acct.next_action,
  ...
  FROM (
    SELECT s.id, s.customer_code, s.company_name, s.owner_email, s.contact_person,
           ...
           s.eff_followup, s.followup_subject, s.next_action, s.days_since_touch,
