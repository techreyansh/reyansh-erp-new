-- Cable Production — operator→employee link + per-operator production efficiency
-- in the Performance score (the "real" MIS feed).
--   1. ppc_wo_stage.operator_email: links a stage's operator to public.employees
--      (kept alongside operator_name for display / back-compat).
--   2. perf_person_week_score gains a renormalized "Production" category: for an
--      employee who ran cable stages that week, efficiency = yield% (good output
--      vs output+scrap) over the stages they COMPLETED. Operators get the
--      category; everyone else has no production data so it renormalizes away —
--      exactly how the other optional categories already behave.
-- Additive (new column) + CREATE OR REPLACE (function). Safe to re-run.
BEGIN;

ALTER TABLE public.ppc_wo_stage ADD COLUMN IF NOT EXISTS operator_email text;
CREATE INDEX IF NOT EXISTS ppc_wo_stage_operator_email_idx
  ON public.ppc_wo_stage (lower(operator_email)) WHERE operator_email IS NOT NULL;
...
DECLARE
  ...
  c_work numeric; c_ontime numeric; c_chk numeric; c_wf numeric; c_meet numeric; c_mgr numeric; c_prod numeric;
  w_work numeric := 0.40; w_ontime numeric := 0.25; w_chk numeric := 0.15; w_wf numeric := 0.10;
  w_meet numeric := 0.05; w_mgr numeric := 0.05; w_prod numeric := 0.15;
...
BEGIN
  ...
  -- PRODUCTION: cable stages this operator COMPLETED in the week. Efficiency =
  -- yield% (good output vs output+scrap). NULL when no quantities recorded, so it
  -- only scores real production and otherwise renormalizes out.
